...
const Staffview = () => {
    ...
    const res=await axios.get("http://localhost:3004/authentication/getstaff")
	setEmp(res.data)
	}
 ...
 setUsername(JSON.parse(storedUsername));
	...
	const deleteStaff = async (id) => {
		...
		if (isConfirmed) {
		try {
		const res = await axios.delete(`http://localhost:3004/authentication/delstaffdata/${id}`);
...
<nav>
            ...
            
		{
				...
				<Link  className='link3'to={`/staffulldetails/${data._id}`} >
